...
extern "C"
int gr_isodecahedron_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  gr_node_ud* data = (gr_node_ud*)lua_newuserdata(L, sizeof(gr_node_ud));
  data->node = 0;
  ...
  const char* name = luaL_checkstring(L, 1);
  data->node = new GeometryNode(name, new Isodecahedron());
...
int gr_material_set_opacity_cmd(lua_State *L)
{
  ...
  const double opacity = luaL_checknumber(L, 2);
  const double ri = luaL_checknumber(L, 3);
...
static const luaL_reg grlib_functions[] = {
  ...
  // Post-A5
  {"isodecahedron", gr_isodecahedron_cmd},
